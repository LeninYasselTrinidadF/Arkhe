    ...
    const char* title, Color title_col, Color border_col, Vector2 mouse) const
{
    ...
    // Fondo
    DrawRectangle(px, py, pw, ph, { 14, 14, 24, 252 });
    DrawRectangleLinesEx({ (float)px, (float)py, (float)pw, (float)ph }, 1.5f, border_col);
    // Cabecera
    DrawRectangle(px, py, pw, 30, { 18, 22, 42, 255 });
    ...
    DrawRectangleRec(cr, chov ? Color{ 180, 50, 50, 255 } : Color{ 50, 40, 60, 255 });
    DrawText("x", (int)cr.x + 7, (int)cr.y + 4, 13, WHITE);
