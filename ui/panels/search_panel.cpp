...
// ── Constantes internas ───────────────────────────────────────────────────────
// Instancia global del panel de búsqueda (declarada en el header)
SearchPanel search_panel;

static constexpr int PAGE_SIZE      = 25;
static constexpr int LOCAL_FULL_MAX = 2000;

// ── Primitivos de dibujo (privados a esta TU) ─────────────────────────────────
...
void SearchPanel::draw(AppState& state, const MathNode* search_root, Vector2 mouse) {
    ...
    Rectangle lf_local = { (float)px, (float)field_y,
                            (float)(pw - local_btn_w - 4), (float)field_h };
    ...
    if (local_focus) {
        ...
        if ((int)strlen(state.search_buf) != prev) {
            local_scroll      = 0.f;
            local_page        = 0;
            local_full_loaded = false;
            local_full_hits.clear();
            local_page_hits.clear();
        }
    }
    ...
    {
        local_full_hits   = fuzzy_search(search_root, state.search_buf, LOCAL_FULL_MAX);
        local_full_loaded = true;
        local_page        = 0;
        local_scroll      = 0.f;
    }
    ...
    if (strlen(state.search_buf) > 0 && search_root && list_h > 0) {
        ...
        if (!local_full_loaded
            && (local_page_hits.empty()
                || local_last_query != std::string(state.search_buf)))
        {
            local_last_query = state.search_buf;
            local_page_hits  = fuzzy_search(search_root, state.search_buf, PAGE_SIZE);
        }

        const std::vector<SearchHit>& all =
            local_full_loaded ? local_full_hits : local_page_hits;

        int  known_total  = (int)all.size();
        bool may_have_more = !local_full_loaded && (known_total >= PAGE_SIZE);
        int  tot_pg        = std::max(1, (known_total + PAGE_SIZE - 1) / PAGE_SIZE);
        if (may_have_more) tot_pg += 1;
        local_page = std::clamp(local_page, 0, tot_pg - 1);

        int page_start = local_page * PAGE_SIZE;
        ...
        local_scroll  = std::clamp(local_scroll, 0.f, max_off);

        bool in_list = panel_active
            && mouse.x > px && mouse.x < px + pw
            && mouse.y >= list_top && mouse.y < list_bot;

        local_scroll = draw_scrollbar(SB_X, list_top, list_h,
                                      cont_h, local_scroll,
                                      mouse, in_list,
        ...
        for (int i = 0; i < page_count; i++) {
            int cy = list_top + i * stride - (int)local_scroll;
        ...
        int new_pg = draw_pager(px, pw, list_bot + item_gap,
                                local_page, tot_pg, mouse, panel_active);
        if (new_pg != local_page) {
            if (!local_full_loaded) {
                local_full_hits   = fuzzy_search(search_root, state.search_buf, LOCAL_FULL_MAX);
                local_full_loaded = true;
                int real_tot = (int)local_full_hits.size();
    ...
    Rectangle lf_loogle = { (float)px, (float)lfld_y,
                             (float)(pw - btn_w - 4), (float)field_h };
    if (panel_active && IsMouseButtonPressed(MOUSE_BUTTON_LEFT))
    ...
    if (state.loogle_loading.load()) {
        DrawTextF("Buscando...", px, ly, 12, th.success);
        ly += g_fonts.scale(14);
    } else if (!state.loogle_error.empty()) {
        DrawTextF(state.loogle_error.c_str(), px, ly, 11, { 200, 100, 100, 255 });
        ly += g_fonts.scale(13);
    }
    ...
    if (!state.loogle_results.empty()) {
        ...
        if (ll_h > 0) {
            ...
            int page_start = loogle_page * PAGE_SIZE;
            ...
            loogle_scroll = draw_scrollbar(SB_X, ll_top, ll_h,
                                           lcont, loogle_scroll,
                                           mouse, in_llist,
            ...
            int new_lp = draw_pager(px, pw, ll_bot + item_gap,
