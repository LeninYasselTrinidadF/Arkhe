    ...
    bool active, Vector2 mouse)
{
    ...
    DrawText(label,x+(w-tw)/2,y+(h-12)/2-1,12,(active||hov)?th.ctrl_text:th_alpha(th.ctrl_text_dim));
...
static bool draw_theme_button(int x, int y, int h, Vector2 mouse) {
    ...
    DrawText(name,x+16,y+(h-11)/2,11,hov?th.ctrl_text:th_alpha(th.ctrl_text_dim));
    return hov&&IsMouseButtonPressed(MOUSE_BUTTON_LEFT);
}
...
bool draw_toolbar(AppState& state, Vector2 mouse) {
    ...
    // Fondo: nine-patch si hay skin, sólido si no
    ...
    DrawLineEx({0.0f,(float)TOOLBAR_H},{(float)w,(float)TOOLBAR_H},1.0f,th_alpha(th.border_panel));
    ...
    DrawText(info.c_str(),w-MeasureText(info.c_str(),10)-12,(TOOLBAR_H-10)/2,10,th_alpha(th.text_dim));
